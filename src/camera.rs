...
impl Camera {
    pub fn ray_color(r: &Ray, depth: usize, world: &HittableList, prng: &mut PRNG<JsfLarge>) -> Color {

        if depth == 0{
            return Color::ZERO;
        }

        match world.hit(r, &Interval::from(0.0000001, f64::MAX)) {
            None => {},
            Some(hr) => {
    ...
    pub fn render(&self, scene: &HittableList) {
        ...
        for j in 0..self.image_height {
            for i in 0..self.image_width {
                ...
                pixel_color = (255.99 * pixel_color).floor().clamp(Color::ZERO, Color::splat(255.0));
                writeln!(writer, "{} {} {}", pixel_color.x, pixel_color.y, pixel_color.z).unwrap();
            }
        }
    }
...
pub fn initialize_camera() -> Camera{
    ...
    let viewport_upper_left = camera_center - V3::new(0.0, 0.0, focal_length) - viewport_u/ 2.0 - viewport_v/2.0;

    let pixel00_loc = viewport_upper_left + 0.5*(pixel_delta_u + pixel_delta_v);

    Camera{image_height, image_width, samples_per_pixel, max_depth, aspect_ratio, camera_center, pixel00_loc, pixel_delta_u, pixel_delta_v}
}