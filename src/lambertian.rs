...
use crate::utility::{random_double, refract, sample_lambertian_scatter, sample_unit_vector};
...
impl Material for Lambertian{
    fn scatter(&self, _r: &Ray, rec: &HitRecord, prng: &mut PRNG<JsfLarge>) -> (bool, Ray, Color) {
        ...
        let scattered = Ray{origin: rec.p, direction: scatter_direction.normalize()};
...
impl Material for Metal{
    fn scatter(&self, r: &Ray, rec: &HitRecord, prng: &mut PRNG<JsfLarge>) -> (bool, Ray, Color) {
        ...
        (rec.normal.dot(scattered.direction) > 0.0, scattered, attenuation)
    }
}
...
impl Material for Dielectric{
    fn scatter(&self, _r: &Ray, rec: &HitRecord, prng: &mut PRNG<JsfLarge>) -> (bool, Ray, Color) {
        ...
        let ri = if rec.is_front_face {1.0 / self.refraction_index} else {self.refraction_index};

        let unit_dir = sample_unit_vector(prng);

        let cos_theta = rec.normal.dot(-unit_dir).min(1.0);
        let sin_theta = cos_theta.mul_add(-cos_theta, 1.0).sqrt();

        let cannot_refract = ri * sin_theta > 1.0;
        let reflectance_cond = self.reflectance(cos_theta) > random_double(prng);

        let direction = if cannot_refract || reflectance_cond {refract(&unit_dir, &rec.normal, self.refraction_index)} else {unit_dir.refract(rec.normal, ri)};
