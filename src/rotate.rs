...
impl RotateY{
    pub fn from(object: Arc<dyn Hittable>, angle: f64) -> Self{
        ...
        Self{object, sin_theta, cos_theta, aabb}
...
impl Hittable for RotateY{
    fn hit(&self, r: &Ray, i: &Interval, prng: &mut PRNG<JsfLarge>) -> Option<HitRecord> {
        ...
        let origin  = P3::new( self.cos_theta * r.origin.x - self.sin_theta*r.origin.z, r.origin.y, self.sin_theta*r.origin.x + self.cos_theta*r.origin.z);
        let direction = V3::new(self.cos_theta * r.direction.x - self.sin_theta*r.direction.z, r.direction.y, self.sin_theta*r.direction.x + self.cos_theta*r.direction.z);

        let rotated_ray = Ray{origin, direction, time: r.time};
        ...
        match hit_rec {
            ...
            Some(mut rec) => {
                ...
                rec.p = P3::new(self.cos_theta * rec.p.x + self.sin_theta*rec.p.z, rec.p.y, -self.sin_theta*rec.p.x + self.cos_theta*rec.p.z);
                rec.normal = V3::new(self.cos_theta *rec.normal.x + self.sin_theta*rec.normal.z, r.origin.y, -self.sin_theta*rec.normal.x + self.cos_theta*rec.normal.z);
                Some(rec)
            }
        }
    }
